...
```bash
pnpm run package        # = node patch.cjs
```
...
> В корне проекта лежит `.npmrc` с `public-hoist-pattern[]=@babel/runtime` — без него
> сборка апстрима падает на Windows (TransformAsyncModulesPlugin не находит
> `@babel/runtime` при изолированной раскладке pnpm). Файл закоммичен вместе с модом,
> при обновлении апстрима его не удалять.

Другие режимы:

```bash
pnpm run patch          # только патч, без сборки (--patch-only)
pnpm run restore        # откатить патчи: git checkout src/ + удалить скопированные файлы (--restore)
```
...
```bash
pnpm run deploy && pnpm run launch
```
...
cd vot-mod && pnpm run restore && cd ..
