...
const VOT_CONFIG_ENTRIES = `${PATCH_MARKER}
  ...
  [
    'votPerfMode',
    { default: true, desc: 'Performance mode (disable UI animations)' }
  ]`;
...
function patchUiJs() {
  ...
  if (!patched.includes(checkboxAnchor)) {
    ...
    patched = patched.replace(
      checkboxAnchor,
      "  elmContainer.appendChild(createConfigCheckbox('votPerfMode'));\n" +
...
function patchHuskyHook() {
  ...
  // to run once the project is switched to npm. Also add a guard that blocks
  // commits of generated paths — they keep sneaking into history otherwise.
  ...
  if (content.includes('pnpm exec')) {
    content = content.replace(/pnpm exec/g, 'npx');
    changed = true;
  }

  const guardMarker = '# @vot-mod generated-paths guard';
  if (!content.includes(guardMarker)) {
    const guard =
      `${guardMarker}\n` +
      "if git diff --cached --name-only | grep -qE '^(src/|assets/|package\\.json$|package-lock\\.json$|tools/deploy\\.js$|\\.husky/)'; then\n" +
      '  echo "BLOCKED: src/, assets/, package.json, tools/ are generated by vot-mod/patch.cjs — commit only vot-mod/."\n' +
      '  echo "Unstage: git reset -- src/ assets/ package.json package-lock.json tools/ .husky/"\n' +
      '  exit 1\n' +
      'fi\n';
